...
//   /* ---------------- UI STATES ---------------- */
//   if (!district || !mandi) {
//     return <Text>{t('select_filter')}</Text>;
//   }
...
export default function GraphChart({ filters }: GraphChartProps) {
  const { district = '', mandi = '', days = 1 } = filters ?? {};
  ...
  if (!district || !mandi) {
    return <Text style={{ color: theme.text }}>{t('select_filter')}</Text>;
  }
  ...
  if (error) {
    return <Text style={{ color: theme.text }}>{error}</Text>;
  }

  if (!data.length) {
    return <Text style={{ color: theme.text }}>No price data available</Text>;
  }

  return (
    <View>
      <Text style={{ fontWeight: '700', marginBottom: 8, color: theme.text }}>
        Price Details ({days} day{days > 1 ? 's' : ''})
      </Text>
      ...
      {data.map((item, index) => (
        ...
        >
          ...
          <Text style={{ color: theme.text }}>
            Predicted Price: ₹{item.pred_price_kg} / kg
          </Text>
          <Text style={{ color: theme.text }}>Min: ₹{item.min_kg}</Text>
          <Text style={{ color: theme.text }}>Max: ₹{item.max_kg}</Text>
          <Text style={{ color: theme.text }}>Average: ₹{item.avg_kg}</Text>
          <Text style={{ color: theme.text }}>
            Daily Change: {item['daily_change_%']}%
          </Text>
          <Text style={{ color: theme.text }}>
            Weekly Change: {item['weekly_change_%']}%
          </Text>
        </View>
      ))}
    </View>
    ...
    <View>
      <BarChart
        ...
        chartConfig={{
          ...
          decimalPlaces: 0,
          color: (opacity = 1) =>
            `rgba(34,197,94, ${opacity})`, // green bars
